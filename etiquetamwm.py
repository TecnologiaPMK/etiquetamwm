...
def create_label_image(data_fabricacao, part_number, nivel_liberacao, serial_fabricacao, nf, logo_path, 
    ...
    width_pixels, height_pixels = (int(label_width * dpi / 25.8), int(label_height * dpi / 25.4))
    ...
    for title, value in info_texts:
        draw.text((220, y_pos), title, fill="black", font=font_title )
        y_pos += 10
        draw.text((220, y_pos), value, fill="black", font=font_data)
        y_pos += 10
    ...
    dm_img = dm_img.resize((15, 150))
    img.paste(dm_img, (2, 30))
