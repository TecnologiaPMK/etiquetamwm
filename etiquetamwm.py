...
def generate_datamatrix(data):
    ...
    qr.save(temp_file.name, scale=40)  # Aumentei a escala para melhor resolução
...
def create_label_image(data_fabricacao, part_number, nivel_liberacao, serial_fabricacao, nf, logo_path, 
    ...
    font_title = ImageFont.truetype(font_path, 24)  # Aumentei o tamanho da fonte
    font_data = ImageFont.truetype(font_path, 24)
    font_code = ImageFont.truetype(font_path, 24)

    # Adiciona o logo
    logo = Image.open(logo_path)
    logo = logo.resize((300, 100))  # Aumentei o tamanho do logo
    ...
    dm_img = dm_img.resize((400, 400))  # Aumentei o tamanho do DataMatrix
    img.paste(dm_img, (50, 150))

    # Código PR
    draw.text((200, 600), PR_datamatrix, fill="black", font=font_code, anchor="mm")
    return img
...
part_number = st.text_input("Part Number MWM:")
nivel_liberacao = st.text_input("Nível de Liberacao:")
serial_fabricacao = st.text_input("Serial de Fabricacao:")
PR_datamatrix = st.text_input("Cod Datamatrix")
...
if st.button("Visualizar Prévia"):
    ...
    st.image(img_preview, caption="Prévia da Etiqueta", width=700)
